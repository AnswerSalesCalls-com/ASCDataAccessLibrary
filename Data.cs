...
namespace ASCTableStorage.Data
{
   ...
   /// with both synchronous and asynchronous support, lambda expressions, proper batch processing, and pagination.
   /// </summary>
   ...
   public class DataAccess<T> where T : TableEntityBase, ITableExtra, new()
   {
      ...
      /// </summary>
      private async Task<CloudTable> GetTableAsync(string tableRef)
      {
         CloudTable ct = m_Client.GetTableReference(tableRef);
      ...
      /// <param name="obj">The Entity to manage</param>
      /// <param name="direction">The direction of the data</param>
      public async Task ManageDataAsync(T obj, TableOperationType direction = TableOperationType.InsertOrReplace)
      {
         _ = obj.GetIDValue(); // Ensures ID always exists
         TableOperation op = CreateTableOperation(obj, direction);
         CloudTable table = await GetTableAsync(obj.TableReference);
      ...
      #region Data Retrieval - Synchronous
      ...
      public T GetRowObject(string rowKeyID)
      {
         ...
         TableQuery<T> q = new TableQuery<T>().Where(queryString);
         return GetCollection(q).FirstOrDefault()!;
      }
      ...
      {
         return GetCollection(predicate).FirstOrDefault()!;
      }

      #endregion

      #region Data Retrieval - Asynchronous

      /// <summary>
      /// Retrieves All Data from the underlying Table (asynchronous)
      /// </summary>
      public async Task<List<T>> GetAllTableDataAsync()
      {
         TableQuery<T> q = new TableQuery<T>();
         return await GetCollectionAsync(q);
      }
      ...
      /// <param name="partitionKeyID">The value that should be found in the PartitionKey space</param>
      /// <returns>A collection of the Type requested from the appropriate table</returns>
      public async Task<List<T>> GetCollectionAsync(string partitionKeyID)
      {
         string queryString = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, partitionKeyID);
         TableQuery<T> q = new TableQuery<T>().Where(queryString);
         return await GetCollectionAsync(q);
      ...
      /// </example>
      public async Task<List<T>> GetCollectionAsync(Expression<Func<T, bool>> predicate)
      {
         string queryString = ConvertLambdaToODataFilter(predicate);
         TableQuery<T> q = new TableQuery<T>().Where(queryString);
         return await GetCollectionAsync(q);
      ...
      /// <param name="combineStyle">How multiple search definitions should be combined</param>
      /// <returns>A collection of the Type requested from the appropriate table</returns>
      public async Task<List<T>> GetCollectionAsync(List<DBQueryItem> queryTerms, QueryCombineStyle combineStyle = QueryCombineStyle.or)
      {
         string queryString = BuildQueryString(queryTerms, combineStyle);
         TableQuery<T> q = new TableQuery<T>().Where(queryString);
         return await GetCollectionAsync(q);
      ...
      /// <param name="definedQuery">The Query Definition to run</param>
      /// <returns>A collection of the Type requested from the appropriate table</returns>
      public async Task<List<T>> GetCollectionAsync(TableQuery<T> definedQuery)
      {
         T obj = Activator.CreateInstance<T>();
         CloudTable table = await GetTableAsync(obj.TableReference);
      ...
      /// </summary>
      /// <param name="rowKeyID">The RowKey in the table to grab</param>
      public async Task<T> GetRowObjectAsync(string rowKeyID)
      {
         string queryString = TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, rowKeyID);
         TableQuery<T> q = new TableQuery<T>().Where(queryString);
         var results = await GetCollectionAsync(q);
      ...
      /// <param name="howToCompare">Direction of the comparison</param>
      /// <param name="fieldValue">The value the field should have</param>
      public async Task<T> GetRowObjectAsync(string fieldName, ComparisonTypes howToCompare, string fieldValue)
      {
         string queryString = $"{fieldName} {howToCompare} '{fieldValue}'";
         TableQuery<T> q = new TableQuery<T>().Where(queryString);
         var results = await GetCollectionAsync(q);
      ...
      /// </example>
      public async Task<T> GetRowObjectAsync(Expression<Func<T, bool>> predicate)
      {
         var results = await GetCollectionAsync(predicate);
         return results.FirstOrDefault();
      }
      ...
      public async Task<PagedResult<T>> GetPagedCollectionAsync(int pageSize = DEFAULT_PAGE_SIZE,
      ...
      public async Task<PagedResult<T>> GetPagedCollectionAsync(string partitionKeyID,
          int pageSize = DEFAULT_PAGE_SIZE, string continuationToken = null)
      {
         string queryString = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, partitionKeyID);
         TableQuery<T> q = new TableQuery<T>().Where(queryString);
         return await GetPagedCollectionAsync(pageSize, continuationToken, q);
      }
      ...
      /// </example>
      public async Task<PagedResult<T>> GetPagedCollectionAsync(Expression<Func<T, bool>> predicate,
          int pageSize = DEFAULT_PAGE_SIZE, string continuationToken = null)
      {
         string queryString = ConvertLambdaToODataFilter(predicate);
         TableQuery<T> q = new TableQuery<T>().Where(queryString);
      ...
      public async Task<PagedResult<T>> GetInitialDataLoadAsync(int initialLoadSize = DEFAULT_PAGE_SIZE,
          TableQuery<T> definedQuery = null)
      {
         return await GetPagedCollectionAsync(initialLoadSize, null, definedQuery);
      }
      ...
      /// <returns>Initial page result</returns>
      public async Task<PagedResult<T>> GetInitialDataLoadAsync(Expression<Func<T, bool>> predicate,
          int initialLoadSize = DEFAULT_PAGE_SIZE)
      {
         return await GetPagedCollectionAsync(predicate, initialLoadSize);
      }
      ...
      public async Task<BatchUpdateResult> BatchUpdateListAsync(List<T> data,
         ...
         try
         {
            ...
            foreach (var group in groupedData)
            {
               ...
               for (int i = 0; i < groupList.Count; i += MAX_BATCH_SIZE)
               {
                  ...
                  try
                  {
                     await ProcessBatchAsync(batchItems, direction);
      ...
      /// <summary>
      /// Synchronous version of batch update (wrapper around async version)
      /// </summary>
      /// <param name="data">The data to work on</param>
      /// <param name="direction">How to put the data into the DB</param>
      ...
      {
         var result = BatchUpdateListAsync(data, direction).GetAwaiter().GetResult();
         return result.Success;
      }

      /// <summary>
      /// Processes a single batch of items with strict size validation
      /// </summary>
      private async Task ProcessBatchAsync(List<T> batchItems, TableOperationType direction)
      {
         ...
         var table = await GetTableAsync(sampleItem.TableReference);
